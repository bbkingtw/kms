...
app.use(function(req,res,next){
	if (req.url.indexOf('/kms')==0) return next();
	if (req.session.dataloaded) return next(); else res.redirect('/kms/config/browse');
	//return res.render('main.jade');
	//load_table(req,res,next);
});
...
app.post('/kms/single_row/new', function(req,res,next){
	...
	next();
}, save_table, load_table, draw_rows);
...
app.post('/kms/single_row/edit/:id', update_row, save_table, load_table, draw_rows);
//app.post('/kms/row/edit/:id', update_row, save_row, reply_row_data);
app.post('/kms/single_row/copy/:id', update_row, save_row, reply_row_data);
...
app.get('/kms/table/save', save_table, draw_rows);
...
function save_table(req,res,next){
	...
	fs.exists(req.config_filename, function(exists){
		if (exists && out.length==0){ 
			file_info(req.config_filename, function(size){
				if (size > 0){
		...
		function go_write() {
			...
			log('start to save',s);
			fs.createWriteStream(req.config_filename).write(s);
...
app.get('/kms/config/load/:file', load_table, draw_rows);
